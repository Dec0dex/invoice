...
import rs.decodex.invoice.model.NavigationModel
...
class SettingsViewController : Initializable {
    ...
    private fun createRootTreeItem(): TreeItem<NavigationModel> {
        val rootItem = TreeItem(NavigationModel(LanguageUtils.getString("settings"), ""))
        rootItem.isExpanded = true
        return rootItem
    }

    private fun createLanguageTreeItem(): TreeItem<NavigationModel> {
        return TreeItem(NavigationModel(LanguageUtils.getString("language"), "settings_language.fxml"))
    }
